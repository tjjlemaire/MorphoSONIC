...
#            yname='amplitude', yfactor=1e-3, yunit='Pa', plot_chr=False)
    ...
    def test_transducer(self, is_profiled=False):
        ...
        psource = PlanarDiskTransducerSource(x0, 0.5*z0, Fdrive, rho=rho, c=c, theta=theta, r=r_tr)
        ...
        # Plot membrane potential traces for specific duration at threshold current
        fig3 = SectionCompTimeSeries([(data, meta)], 'Qm', fiber.ids).render()
        fig4 = SectionCompTimeSeries([(data, meta)], 'Vm', fiber.ids).render()

        fiber.reset()
        
                # Create ultrasound source
        psource = PlanarDiskTransducerSource(x0, 2*z0, Fdrive, rho=rho, c=c, theta=theta, r=r_tr)
        ...
        # Plot membrane potential traces for specific duration at threshold current
        fig5 = SectionCompTimeSeries([(data, meta)], 'Qm', fiber.ids).render()
        fig6 = SectionCompTimeSeries([(data, meta)], 'Vm', fiber.ids).render()
